...
public:
    class iterator // забележете - итераторът е в public частта!
    {
        friend class BST<T, Comp>;
        
        // итераторът съдържа само един указател към кутийка, но за почти
        // всякакви цели се държи като указател към съдържания тип Т.
        const Node* ptr;

        // само дървото ще конструира итератори, пък ние ще работим с тях с другите им методи
        iterator(const Node*);
    public:
        // тези методи са премахнати, понеже не променяме стойностите в дървото (!)
        //T& operator*();
        //T* operator->();
        const T& operator*() const;
        const T* operator->() const;
        iterator& operator++();   // prefix, връща референция към себе си
        iterator operator++(int); // postfix, връща свое копие, dummy аргумент
        iterator& operator--();
    ...
    BST(const Comp& = Comp());
    BST(const BST&) = delete; // will this ever get done...
...
BST<T, Comp>::iterator::iterator(const Node* _p) : ptr(_p) {}

template<class T, class Comp>
const T& BST<T, Comp>::iterator::operator*() const
{
    return ptr->value;
}
...
template<class T, class Comp>
auto BST<T, Comp>::iterator::operator++() -> iterator&
{
    if (ptr->right)
    {
        // ако има поне един десен наследник, намира най-малкия си десен наследник
        ptr = ptr->right;
        while (ptr->left)
            ptr = ptr->left;
    }
    ...
    {
        // в противен случая намира най-малкият си десен предшественик (прародител)
...
template<class T, class Comp>
auto BST<T, Comp>::iterator::operator++(int) -> iterator
{
    // постфиксните версии винаги изпозлват префиксните наготово
    iterator copy = *this;
    ++(*this);
    return copy;
}
...
auto BST<T, Comp>::iterator::operator--() -> iterator&
{
    // за декрементиране на итератор процедурата е същата, просто симетрична
    if (ptr->left)
    {
        ptr = ptr->left;
        while (ptr->right)
...
template<class T, class Comp>
auto BST<T, Comp>::begin() const -> iterator
{
    // невалиден итератор <=> невалиден, т.е. нулев указател
    if (!root)
        return iterator(nullptr);
    // първият елемент е този, който се намира най-вляво от корена
    const Node* location = root;
    while (location->left)
        location = location->left;
    // конструираме итератор (както само ние можем) и го връщаме
    return iterator(location);
}

template<class T, class Comp>
auto BST<T, Comp>::end() const -> iterator
{
    // итератор за след последния елемент, т.е. на невалидна позиция
    return iterator(nullptr);
}
